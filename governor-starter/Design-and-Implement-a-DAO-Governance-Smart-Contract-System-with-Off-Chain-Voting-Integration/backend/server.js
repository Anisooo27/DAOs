  ...
  try {
    ...
    const result = await withSignerLock(async () => {
      ...
      try {
        ...
        if (n > 0 && String(value).length <= 10) proposalValue = ethers.parseEther(String(value));
        else if (n > 0) proposalValue = BigInt(value);
      ...
      try {
        const { tx: qTx } = await nm.sendTx(governor, 'queue', [[target], [proposalValue], [calldata], descriptionHash]);
      ...
      const { tx: exTx, receipt } = await nm.sendTx(governor, 'execute', [[target], [proposalValue], [calldata], descriptionHash]);
